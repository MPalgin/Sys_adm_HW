...
```
select sum(amount), monthname(payment_date), count(rental_id) from payment group by monthname(payment_date)
having sum(amount) >= all(select sum(amount) from payment group by monthname(payment_date));
